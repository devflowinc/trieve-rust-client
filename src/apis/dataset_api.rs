...
pub async fn get_all_tags(configuration: &configuration::Configuration, tr_dataset: &str, get_all_tags_req_payload: models::GetAllTagsReqPayload) -> Result<models::GetAllTagsResponse, Error<GetAllTagsError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/dataset/get_all_tags", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    local_var_req_builder = local_var_req_builder.json(&get_all_tags_req_payload);
